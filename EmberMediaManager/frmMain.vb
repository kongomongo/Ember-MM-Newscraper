...
#Region "Fields"
    ...
    Private MainActor As New Images
...
#Region "Methods"
    ...
    Private Sub bwMovieScraper_Completed(ByVal sender As Object, ByVal e As System.ComponentModel.RunWorkerCompletedEventArgs) Handles bwMovieScraper.RunWorkerCompleted
        ...
        If Res.DBElement.ScrapeType = Enums.ScrapeType.SingleScrape AndAlso Not Res.Cancelled Then
    ...
    Private Sub bwMovieScraper_DoWork(ByVal sender As Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles bwMovieScraper.DoWork
        ...
        For Each tScrapeItem As Database.DBElement In Args.ScrapeList
            ...
            DBScrapeMovie = Master.DB.Load_Movie(tScrapeItem.ID)
            DBScrapeMovie.ScrapeModifiers = tScrapeItem.ScrapeModifiers
            DBScrapeMovie.ScrapeOptions = tScrapeItem.ScrapeOptions
            DBScrapeMovie.ScrapeType = tScrapeItem.ScrapeType
            ...
            Else
                ...
                If Not DBScrapeMovie.MainDetails.AnyUniqueIDSpecified AndAlso (tScrapeItem.ScrapeModifiers.MainActorthumbs Or tScrapeItem.ScrapeModifiers.MainBanner Or tScrapeItem.ScrapeModifiers.MainClearArt Or
            ...
            If Not Cancelled Then
                If Master.eSettings.MovieScraperMetaDataScan AndAlso tScrapeItem.ScrapeModifiers.MainMeta Then
                    ...
                    MetaData.ScanMetaData(DBScrapeMovie)
                End If
                If bwMovieScraper.CancellationPending Then Exit For
                ...
                If tScrapeItem.ScrapeModifiers.MainTheme Then
                    ...
                    If AddonsManager.Instance.ScrapeTheme_Movie(DBScrapeMovie, Enums.ScrapeModifierType.MainTheme, SearchResults) Then
                        If tScrapeItem.ScrapeType = Enums.ScrapeType.SingleScrape Then
                            ...
                            'autoscraping
                        ElseIf Not tScrapeItem.ScrapeType = Enums.ScrapeType.SingleScrape Then
                ...
                If Not (tScrapeItem.ScrapeType = Enums.ScrapeType.SingleScrape) Then
                    AddonsManager.Instance.RunGeneric(Enums.AddonEventType.DuringScrapingMulti_Movie, Nothing, Nothing, False, DBScrapeMovie)
        ...
        e.Result = New Results With {.DBElement = DBScrapeMovie, .Cancelled = bwMovieScraper.CancellationPending}
    ...
    Private Sub bwMovieSetScraper_DoWork(ByVal sender As Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles bwMovieSetScraper.DoWork
        ...
        For Each tScrapeItem As Database.DBElement In Args.ScrapeList
            ...
            DBScrapeMovieSet = Master.DB.Load_MovieSet(tScrapeItem.ID)
            DBScrapeMovieSet.ScrapeModifiers = tScrapeItem.ScrapeModifiers
            DBScrapeMovieSet.ScrapeOptions = tScrapeItem.ScrapeOptions
            DBScrapeMovieSet.ScrapeType = tScrapeItem.ScrapeType

            OldListTitle = DBScrapeMovieSet.ListTitle
            OldTitle = DBScrapeMovieSet.MainDetails.Title
            OldTMDBColID = DBScrapeMovieSet.MainDetails.TMDBColID
            ...
            If Not Cancelled Then
                ...
                NewTMDBColID = DBScrapeMovieSet.MainDetails.TMDB
        ...
        e.Result = New Results With {.DBElement = DBScrapeMovieSet, .Cancelled = bwMovieSetScraper.CancellationPending}
    ...
    Private Sub bwTVScraper_DoWork(ByVal sender As Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles bwTVScraper.DoWork
        ...
        For Each tScrapeItem As Database.DBElement In Args.ScrapeList
            ...
            DBScrapeShow = Master.DB.Load_TVShow_Full(tScrapeItem.ID)
            DBScrapeShow.ScrapeModifiers = tScrapeItem.ScrapeModifiers
            DBScrapeShow.ScrapeOptions = tScrapeItem.ScrapeOptions
            DBScrapeShow.ScrapeType = tScrapeItem.ScrapeType

            OldListTitle = DBScrapeShow.ListTitle
            ...
            If Not Cancelled Then
                ...
                If tScrapeItem.ScrapeModifiers.withEpisodes AndAlso tScrapeItem.ScrapeModifiers.EpisodeMeta AndAlso Master.eSettings.TVScraperMetaDataScan Then
                    ...
                    For Each tEpisode In DBScrapeShow.Episodes.Where(Function(f) f.FilenameSpecified)
                        MetaData.ScanMetaData(tEpisode)
                    Next
                End If

                If Not (tScrapeItem.ScrapeType = Enums.ScrapeType.SingleScrape) Then
                    AddonsManager.Instance.RunGeneric(Enums.AddonEventType.DuringScrapingMulti_TVShow, Nothing, Nothing, False, DBScrapeShow)
        ...
        e.Result = New Results With {.DBElement = DBScrapeShow, .Cancelled = bwTVScraper.CancellationPending}
    ...
    Private Sub bwTVEpisodeScraper_DoWork(ByVal sender As Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles bwTVEpisodeScraper.DoWork
        ...
        For Each tScrapeItem As Database.DBElement In Args.ScrapeList
            ...
            DBScrapeEpisode = Master.DB.Load_TVEpisode(tScrapeItem.ID, True)
            DBScrapeEpisode.ScrapeModifiers = tScrapeItem.ScrapeModifiers
            DBScrapeEpisode.ScrapeOptions = tScrapeItem.ScrapeOptions
            DBScrapeEpisode.ScrapeType = tScrapeItem.ScrapeType

            OldEpisodeTitle = DBScrapeEpisode.MainDetails.Title
            ...
            If Not Cancelled Then
                If Master.eSettings.TVScraperMetaDataScan AndAlso tScrapeItem.ScrapeModifiers.EpisodeMeta Then
                    MetaData.ScanMetaData(DBScrapeEpisode)
                End If
                If bwTVEpisodeScraper.CancellationPending Then Exit For
                ...
                If Not (tScrapeItem.ScrapeType = Enums.ScrapeType.SingleScrape) Then
                    AddonsManager.Instance.RunGeneric(Enums.AddonEventType.DuringScrapingMulti_TVEpisode, Nothing, Nothing, False, DBScrapeEpisode)
        ...
        e.Result = New Results With {.DBElement = DBScrapeEpisode, .Cancelled = bwTVEpisodeScraper.CancellationPending}
    ...
    Private Sub bwTVSeasonScraper_DoWork(ByVal sender As Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles bwTVSeasonScraper.DoWork
        ...
        For Each tScrapeItem As Database.DBElement In Args.ScrapeList
            ...
            DBScrapeSeason = Master.DB.Load_TVSeason(tScrapeItem.ID, True, False)
            DBScrapeSeason.ScrapeModifiers = tScrapeItem.ScrapeModifiers
            DBScrapeSeason.ScrapeOptions = tScrapeItem.ScrapeOptions
            DBScrapeSeason.ScrapeType = tScrapeItem.ScrapeType
            ...
            If Not Cancelled Then
                ...
                If Not (tScrapeItem.ScrapeType = Enums.ScrapeType.SingleScrape) Then
                    AddonsManager.Instance.RunGeneric(Enums.AddonEventType.DuringScrapingMulti_TVSeason, Nothing, Nothing, False, DBScrapeSeason)
        ...
        e.Result = New Results With {.DBElement = DBScrapeSeason, .Cancelled = bwTVSeasonScraper.CancellationPending}
                ...
                If dlgChangeEp.ShowDialog = DialogResult.OK Then
                    If dlgChangeEp.Result.Count > 0 Then
                        If Master.eSettings.TVScraperMetaDataScan Then
                            MetaData.ScanMetaData(tmpEpisode)
                        End If
                        ...
                        If Not Convert.ToInt64(sRow.Cells("idFile").Value) = -1 Then
                            ...
                            If Not String.IsNullOrEmpty(ePath) Then
                                Using Explorer As New Process
                                    ...
                                    Dim strPath As String = FileUtils.Stacking.GetFirstStackedPath(ePath)

                                    If Master.isWindows Then
                                        Explorer.StartInfo.FileName = "explorer.exe"
                                        Explorer.StartInfo.Arguments = String.Format("/select,""{0}""", strPath)
                                    Else
                                        Explorer.StartInfo.FileName = "xdg-open"
                                        Explorer.StartInfo.Arguments = String.Format("""{0}""", strPath)
                                    End If
                                    Explorer.Start()
                                End Using
                            End If
    ...
    Private Sub cmnuEpisodeReload_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmnuEpisodeReload.Click
        ...
        If doFill Then FillListTVEpisodes(Convert.ToInt64(dgvTVEpisodes.SelectedRows(0).Cells("idEpisode").Value), Convert.ToInt32(dgvTVEpisodes.SelectedRows(0).Cells("Season").Value))
    End Sub
    ...
    Private Sub cmnuSeasonReload_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmnuSeasonReload.Click
        ...
        If doFill Then FillListTVSeasons(Convert.ToInt64(dgvTVSeasons.SelectedRows(0).Cells("idShow").Value))
    End Sub
            ...
            Using dEditMovie As New dlgEdit
                ...
                Select Case dEditMovie.ShowDialog(DBMovie)
                    Case DialogResult.OK
                        ...
                        AddonsManager.Instance.RunGeneric(EventType, Nothing, Nothing, False, DBMovie)
    ...
    Private Sub Edit_MovieSet(ByRef DBMovieSet As Database.DBElement)
        ...
        Using dEditMovieSet As New dlgEdit
            AddonsManager.Instance.RunGeneric(Enums.AddonEventType.BeforeEdit_MovieSet, Nothing, Nothing, False, DBMovieSet)
            ...
            Select Case dEditMovieSet.ShowDialog(DBMovieSet)
                Case DialogResult.OK
                    ...
                    AddonsManager.Instance.RunGeneric(Enums.AddonEventType.AfterEdit_MovieSet, Nothing, Nothing, False, DBMovieSet)
                    tslLoading.Text = String.Concat(Master.eLang.GetString(399, "Downloading and Saving Contents into Database"), ":")
            ...
            Using dEditTVEpisode As New dlgEdit
                ...
                Select Case dEditTVEpisode.ShowDialog(DBTVEpisode)
                    Case DialogResult.OK
                        ...
                        AddonsManager.Instance.RunGeneric(EventType, Nothing, Nothing, False, DBTVEpisode)
                        tslLoading.Text = String.Concat(Master.eLang.GetString(399, "Downloading and Saving Contents into Database"), ":")
            ...
            Using dEditTVSeason As New dlgEdit
                ...
                Select Case dEditTVSeason.ShowDialog(DBTVSeason)
                    Case DialogResult.OK
                        ...
                        AddonsManager.Instance.RunGeneric(EventType, Nothing, Nothing, False, DBTVSeason)
                        tslLoading.Text = String.Concat(Master.eLang.GetString(399, "Downloading and Saving Contents into Database"), ":")
            ...
            Using dEditTVShow As New dlgEdit
                ...
                Select Case dEditTVShow.ShowDialog(DBTVShow)
                    Case DialogResult.OK
                        ...
                        AddonsManager.Instance.RunGeneric(EventType, Nothing, Nothing, False, DBTVShow)
                        tslLoading.Text = String.Concat(Master.eLang.GetString(399, "Downloading and Saving Contents into Database"), ":")
    ...
    Private Sub FillListTVSeasons(ByVal ShowID As Long)
        ...
        If dtTVSeasons.Rows.Count > 0 Then
            ...
            FillListTVEpisodes(ShowID, Convert.ToInt32(dgvTVSeasons.Item("Season", 0).Value))
        End If
        AddHandler dgvTVSeasons.SelectionChanged, AddressOf dgvTVSeasons_SelectionChanged
    End Sub

    Private Sub FillListTVEpisodes(ByVal ShowID As Long, ByVal Season As Integer)
    ...
    Private Sub FillScreenInfoWith_Movie()
        SuspendLayout()
        If currMovie.MainDetails.TitleSpecified AndAlso currMovie.MainDetails.YearSpecified Then
            lblTitle.Text = String.Format("{0} ({1})", currMovie.MainDetails.Title, currMovie.MainDetails.Year)
        ElseIf currMovie.MainDetails.TitleSpecified AndAlso Not currMovie.MainDetails.YearSpecified Then
            lblTitle.Text = currMovie.MainDetails.Title
        ElseIf Not currMovie.MainDetails.TitleSpecified AndAlso currMovie.MainDetails.YearSpecified Then
            lblTitle.Text = String.Format(Master.eLang.GetString(117, "Unknown Movie ({0})"), currMovie.MainDetails.Year)
        End If
        ...
        Try
            If currMovie.MainDetails.RatingSpecified Then
                If currMovie.MainDetails.VotesSpecified Then
                    Dim strRating As String = Double.Parse(currMovie.MainDetails.Rating, Globalization.CultureInfo.InvariantCulture).ToString("N1", Globalization.CultureInfo.CurrentCulture)
                    Dim strVotes As String = Double.Parse(currMovie.MainDetails.Votes, Globalization.CultureInfo.InvariantCulture).ToString("N0", Globalization.CultureInfo.CurrentCulture)
            ...
            End If
        Catch ex As Exception
            logger.Error(String.Concat("Error: Not valid Rating or Votes (", currMovie.MainDetails.Rating, " / ", currMovie.MainDetails.Votes, ")"))
            lblRating.Text = "Error: Please rescrape Rating"
        End Try

        If currMovie.MainDetails.RuntimeSpecified Then
            lblRuntime.Text = String.Format(Master.eLang.GetString(112, "Runtime: {0}"), If(currMovie.MainDetails.Runtime.Contains("|"), Microsoft.VisualBasic.Strings.Left(currMovie.MainDetails.Runtime, currMovie.MainDetails.Runtime.IndexOf("|")), currMovie.MainDetails.Runtime)).Trim
        End If

        If currMovie.MainDetails.Top250Specified Then
            pnlTop250.Visible = True
            lblTop250.Text = currMovie.MainDetails.Top250.ToString
        Else
            pnlTop250.Visible = False
        End If
        ...
        Dim tmpRating As Single = NumUtils.ConvertToSingle(currMovie.MainDetails.Rating)
        If tmpRating > 0 Then
            BuildStars(tmpRating)
        End If

        If currMovie.MainDetails.GenresSpecified Then
            createGenreThumbs(currMovie.MainDetails.Genres)
        End If

        If currMovie.MainDetails.StudiosSpecified Then
            pbStudio.Image = APIXML.GetStudioImage(currMovie.MainDetails.Studios.Item(0).ToLower) 'ByDef all images file a lower case
        ...
        txtTMDBID.Text = If(currMovie.MainDetails.TMDBSpecified, CStr(currMovie.MainDetails.TMDB), String.Empty)
        ...
        txtCertifications.Text = String.Join(" / ", currMovie.MainDetails.Certifications.ToArray)
    ...
    Private Sub FillScreenInfoWith_TVEpisode()
        ...
        Try
            If currTV.MainDetails.RatingSpecified Then
                If currTV.MainDetails.VotesSpecified Then
                    Dim strRating As String = Double.Parse(currTV.MainDetails.Rating, Globalization.CultureInfo.InvariantCulture).ToString("N1", Globalization.CultureInfo.CurrentCulture)
                    Dim strVotes As String = Double.Parse(currTV.MainDetails.Votes, Globalization.CultureInfo.InvariantCulture).ToString("N0", Globalization.CultureInfo.CurrentCulture)
                    lblRating.Text = String.Concat(strRating, "/10 (", String.Format(Master.eLang.GetString(118, "{0} Votes"), strVotes), ")")
                Else
                    Dim strRating As String = Double.Parse(currTV.MainDetails.Rating, Globalization.CultureInfo.InvariantCulture).ToString("N1", Globalization.CultureInfo.CurrentCulture)
                    lblRating.Text = String.Concat(strRating, "/10")
            ...
            End If
        Catch ex As Exception
            logger.Error(String.Concat("Error: Not valid Rating or Votes (", currTV.MainDetails.Rating, " / ", currTV.MainDetails.Votes, ")"))
            lblRating.Text = "Error: Please rescrape Rating"
        End Try
        ...
        Dim tmpRating As Single = NumUtils.ConvertToSingle(currTV.MainDetails.Rating)
        If tmpRating > 0 Then
            BuildStars(tmpRating)
        End If
        ...
        txtMetaData.Text = NFO.FIToString(currTV.MainDetails.FileInfo, True)
    ...
    Private Sub FillScreenInfoWith_TVShow()
        ...
        txtPlot.Text = currTV.MainDetails.Plot
        lblRuntime.Text = String.Format(Master.eLang.GetString(645, "Premiered: {0}"), If(currTV.MainDetails.PremieredSpecified, Date.Parse(currTV.MainDetails.Premiered).ToShortDateString, "?"))
        ...
        If currTV.MainDetails.Genres.Count > 0 Then
            createGenreThumbs(currTV.MainDetails.Genres)
        End If

        If currTV.MainDetails.StudiosSpecified Then
            pbStudio.Image = APIXML.GetStudioImage(currTV.MainDetails.Studios.Item(0).ToLower) 'ByDef all images file a lower case
    ...
    Private Sub frmMain_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        AddonsManager.Instance.RuntimeObjects.MediaTabSelected = DirectCast(tcMain.SelectedTab.Tag, Structures.MainTabType)

        AddonsManager.Instance.RuntimeObjects.DelegateLoadMedia(AddressOf LoadMedia)
        AddonsManager.Instance.RuntimeObjects.DelegateOpenImageViewer(AddressOf OpenImageViewer)
        AddonsManager.Instance.RuntimeObjects.MainMenu = mnuMain
        AddonsManager.Instance.RuntimeObjects.MainTabControl = tcMain
        ...
        While Not AddonsManager.Instance.ModulesLoaded()
                ...
                Using SQLtransaction As SQLite.SQLiteTransaction = Master.DB.MyVideosDBConn.BeginTransaction()
                    Select Case _SelectedContentType
                        Case "movie"
                            For Each sRow As DataGridViewRow In dgvMovies.SelectedRows
                                Dim tmpDBElement As Database.DBElement = Master.DB.Load_Movie(Convert.ToInt64(sRow.Cells("idMovie").Value))
                                tmpDBElement.MainDetails.Trailer = strNewValue
                                Master.DB.Save_Movie(tmpDBElement, True, True, False, True, False)
                                RefreshRow_Movie(tmpDBElement.ID)
                            Next
                    End Select
                    ...
                    Select Case _SelectedContentType
                        Case "movie"
                            For Each sRow As DataGridViewRow In dgvMovies.SelectedRows
                                ...
                                tmpDBElement.MainDetails.VideoSource = strNewValue
                                Master.DB.Save_Movie(tmpDBElement, True, True, False, True, False)
                                RefreshRow_Movie(tmpDBElement.ID)
                            Next
                        Case "tvepisode"
    ...
    Private Sub lstActors_SelectedValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles lstActors.SelectedValueChanged
        If lstActors.Items.Count > 0 AndAlso lstActors.SelectedItems.Count > 0 AndAlso alActors.Item(lstActors.SelectedIndex) IsNot Nothing AndAlso Not alActors.Item(lstActors.SelectedIndex).ToString = "none" Then
            ...
            If Not alActors.Item(lstActors.SelectedIndex).ToString.Trim.StartsWith("http") Then
                MainActor.LoadFromFile(alActors.Item(lstActors.SelectedIndex).ToString, True)

                If MainActor.Image IsNot Nothing Then
                    pbActors.Image = MainActor.Image
                Else
                    pbActors.Image = My.Resources.actor_silhouette
                End If
            Else
        ...
        With Master.eSettings
            Select Case _SelectedContentType
                Case "movie"
                    ...
                    mnuScrapeModifierBanner.Enabled = .FilenameAnyEnabled_Movie_Banner AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainBanner)
                    ...
                    mnuScrapeModifierClearArt.Enabled = .FilenameAnyEnabled_Movie_ClearArt AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainClearArt)
                    mnuScrapeModifierClearArt.Visible = True
                    mnuScrapeModifierClearLogo.Enabled = .FilenameAnyEnabled_Movie_ClearLogo AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainClearLogo)
                    mnuScrapeModifierClearLogo.Visible = True
                    mnuScrapeModifierDiscArt.Enabled = .FilenameAnyEnabled_Movie_DiscArt AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainDiscArt)
                    mnuScrapeModifierDiscArt.Visible = True
                    mnuScrapeModifierExtrafanarts.Enabled = .FilenameAnyEnabled_Movie_Extrafanarts AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainExtrafanarts)
                    mnuScrapeModifierExtrafanarts.Visible = True
                    mnuScrapeModifierExtrathumbs.Enabled = .FilenameAnyEnabled_Movie_Extrathumbs AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainExtrathumbs)
                    mnuScrapeModifierExtrathumbs.Visible = True
                    mnuScrapeModifierFanart.Enabled = .FilenameAnyEnabled_Movie_Fanart AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainFanart)
                    mnuScrapeModifierFanart.Visible = True
                    mnuScrapeModifierLandscape.Enabled = .FilenameAnyEnabled_Movie_Landscape AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainLandscape)
                    ...
                    mnuScrapeModifierPoster.Enabled = .FilenameAnyEnabled_Movie_Poster AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainPoster)
                    mnuScrapeModifierPoster.Visible = True
                    mnuScrapeModifierTheme.Enabled = .FilenameAnyEnabled_Movie_Theme AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Theme_Movie(Enums.ScrapeModifierType.MainTheme)
                    mnuScrapeModifierTheme.Visible = True
                    mnuScrapeModifierTrailer.Enabled = .FilenameAnyEnabled_Movie_Trailer AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Trailer_Movie(Enums.ScrapeModifierType.MainTrailer)
                ...
                Case "movieset"
                    ...
                    mnuScrapeModifierBanner.Enabled = .FilenameAnyEnabled_MovieSet_Banner AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_MovieSet(Enums.ScrapeModifierType.MainBanner)
                    mnuScrapeModifierBanner.Visible = True
                    mnuScrapeModifierCharacterArt.Enabled = False
                    mnuScrapeModifierCharacterArt.Visible = False
                    mnuScrapeModifierClearArt.Enabled = .FilenameAnyEnabled_MovieSet_ClearArt AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_MovieSet(Enums.ScrapeModifierType.MainClearArt)
                    mnuScrapeModifierClearArt.Visible = True
                    mnuScrapeModifierClearLogo.Enabled = .FilenameAnyEnabled_MovieSet_ClearLogo AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_MovieSet(Enums.ScrapeModifierType.MainClearLogo)
                    mnuScrapeModifierClearLogo.Visible = True
                    mnuScrapeModifierDiscArt.Enabled = .FilenameAnyEnabled_MovieSet_DiscArt AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_MovieSet(Enums.ScrapeModifierType.MainDiscArt)
                    mnuScrapeModifierDiscArt.Visible = True
                    ...
                    mnuScrapeModifierFanart.Enabled = .FilenameAnyEnabled_MovieSet_Fanart AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_MovieSet(Enums.ScrapeModifierType.MainFanart)
                    mnuScrapeModifierFanart.Visible = True
                    mnuScrapeModifierLandscape.Enabled = .FilenameAnyEnabled_MovieSet_Landscape AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_MovieSet(Enums.ScrapeModifierType.MainLandscape)
                    mnuScrapeModifierLandscape.Visible = True
                    mnuScrapeModifierMetaData.Enabled = False
                    mnuScrapeModifierMetaData.Visible = False
                    mnuScrapeModifierNFO.Enabled = True
                    mnuScrapeModifierNFO.Visible = True
                    mnuScrapeModifierPoster.Enabled = .FilenameAnyEnabled_MovieSet_Poster AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_MovieSet(Enums.ScrapeModifierType.MainPoster)
                    mnuScrapeModifierPoster.Visible = True
                ...
                Case "tvshow"
                    ...
                    mnuScrapeModifierActorthumbs.Visible = True
                    mnuScrapeModifierBanner.Enabled = .FilenameAnyEnabled_TVShow_Banner AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_TV(Enums.ScrapeModifierType.MainBanner)
                    mnuScrapeModifierBanner.Visible = True
                    mnuScrapeModifierCharacterArt.Enabled = .FilenameAnyEnabled_TVShow_CharacterArt AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_TV(Enums.ScrapeModifierType.MainCharacterArt)
                    mnuScrapeModifierCharacterArt.Visible = True
                    mnuScrapeModifierClearArt.Enabled = .FilenameAnyEnabled_TVShow_ClearArt AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_TV(Enums.ScrapeModifierType.MainClearArt)
                    mnuScrapeModifierClearArt.Visible = True
                    mnuScrapeModifierClearLogo.Enabled = .FilenameAnyEnabled_TVShow_ClearLogo AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_TV(Enums.ScrapeModifierType.MainClearLogo)
                    mnuScrapeModifierClearLogo.Visible = True
                    mnuScrapeModifierDiscArt.Enabled = False
                    mnuScrapeModifierDiscArt.Visible = False
                    mnuScrapeModifierExtrafanarts.Enabled = .FilenameAnyEnabled_TVShow_Extrafanarts AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_TV(Enums.ScrapeModifierType.MainExtrafanarts)
                    mnuScrapeModifierExtrafanarts.Visible = True
                    mnuScrapeModifierExtrathumbs.Enabled = False
                    mnuScrapeModifierExtrathumbs.Visible = False
                    mnuScrapeModifierFanart.Enabled = .FilenameAnyEnabled_TVShow_Fanart AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_TV(Enums.ScrapeModifierType.MainFanart)
                    mnuScrapeModifierFanart.Visible = True
                    mnuScrapeModifierLandscape.Enabled = .FilenameAnyEnabled_TVShow_Landscape AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_TV(Enums.ScrapeModifierType.MainLandscape)
                    mnuScrapeModifierLandscape.Visible = True
                    mnuScrapeModifierMetaData.Enabled = False
                    mnuScrapeModifierMetaData.Visible = False
                    mnuScrapeModifierNFO.Enabled = True
                    mnuScrapeModifierNFO.Visible = True
                    mnuScrapeModifierPoster.Enabled = .FilenameAnyEnabled_TVShow_Poster AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_TV(Enums.ScrapeModifierType.MainPoster)
                    mnuScrapeModifierPoster.Visible = True
                    mnuScrapeModifierTheme.Enabled = .FilenameAnyEnabled_TVShow_Theme AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Theme_TV(Enums.ScrapeModifierType.MainTheme)
                    mnuScrapeModifierTheme.Visible = True
                    mnuScrapeModifierTrailer.Enabled = False
                    mnuScrapeModifierTrailer.Visible = False
            End Select
    ...
    Private Sub InfoDownloaded_Movie(ByRef DBMovie As Database.DBElement)
        If Not String.IsNullOrEmpty(DBMovie.MainDetails.Title) Then
            ...
            Edit_Movie(DBMovie, Enums.AddonEventType.DuringScrapingSingle_Movie)
        End If
    ...
    Private Sub CreateScrapeList_Movie(ByVal tScrapeType As Enums.ScrapeType, ByVal tScrapeOptions As Structures.ScrapeOptions, ByVal tScrapeModifiers As Structures.ScrapeModifiers)
        Dim DataRowList As New List(Of DataRow)
        Dim ScrapeList As New List(Of Database.DBElement)
        ...
        Dim BannerAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_Movie_Banner AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainBanner)
        Dim ClearArtAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_Movie_ClearArt AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainClearArt)
        Dim ClearLogoAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_Movie_ClearLogo AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainClearLogo)
        Dim DiscArtAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_Movie_DiscArt AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainDiscArt)
        Dim ExtrafanartsAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_Movie_Extrafanarts AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainFanart)
        Dim ExtrathumbsAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_Movie_Extrathumbs AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainFanart)
        Dim FanartAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_Movie_Fanart AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainFanart)
        Dim LandscapeAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_Movie_Landscape AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainLandscape)
        Dim PosterAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_Movie_Poster AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_Movie(Enums.ScrapeModifierType.MainPoster)
        Dim ThemeAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_Movie_Theme AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Theme_Movie(Enums.ScrapeModifierType.MainTheme)
        Dim TrailerAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_Movie_Trailer AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Trailer_Movie(Enums.ScrapeModifierType.MainTrailer)
        ...
        For Each drvRow As DataRow In DataRowList
            ...
            sModifier.MainActorthumbs = tScrapeModifiers.MainActorthumbs AndAlso ActorThumbsAllowed
            sModifier.MainBanner = tScrapeModifiers.MainBanner AndAlso BannerAllowed
            sModifier.MainClearArt = tScrapeModifiers.MainClearArt AndAlso ClearArtAllowed
            sModifier.MainClearLogo = tScrapeModifiers.MainClearLogo AndAlso ClearLogoAllowed
            sModifier.MainDiscArt = tScrapeModifiers.MainDiscArt AndAlso DiscArtAllowed
            sModifier.MainExtrafanarts = tScrapeModifiers.MainExtrafanarts AndAlso ExtrafanartsAllowed
            sModifier.MainExtrathumbs = tScrapeModifiers.MainExtrathumbs AndAlso ExtrathumbsAllowed
            sModifier.MainFanart = tScrapeModifiers.MainFanart AndAlso FanartAllowed
            sModifier.MainLandscape = tScrapeModifiers.MainLandscape AndAlso LandscapeAllowed
            sModifier.MainMeta = tScrapeModifiers.MainMeta
            sModifier.MainNFO = tScrapeModifiers.MainNFO
            sModifier.MainPoster = tScrapeModifiers.MainPoster AndAlso PosterAllowed
            'sModifier.MainSubtitles = ScrapeModifier.MainSubtitles AndAlso SubtitlesAllowed
            sModifier.MainTheme = tScrapeModifiers.MainTheme AndAlso ThemeAllowed
            sModifier.MainTrailer = tScrapeModifiers.MainTrailer AndAlso TrailerAllowed
            ...
            End Select
            ScrapeList.Add(New Database.DBElement(Enums.ContentType.Movie) With {
                           .ID = CLng(drvRow.Item("idMovie")),
        ...
        If Not ScrapeList.Count = 0 Then
            ...
            If Not tScrapeType = Enums.ScrapeType.SingleScrape Then
            ...
            bwMovieScraper.RunWorkerAsync(New Arguments With {.ScrapeList = ScrapeList})
        End If
    End Sub
    ...
    Private Sub CreateScrapeList_MovieSet(ByVal tScrapeType As Enums.ScrapeType, ByVal tScrapeOptions As Structures.ScrapeOptions, ByVal tScrapeModifiers As Structures.ScrapeModifiers)
        Dim DataRowList As New List(Of DataRow)
        Dim ScrapeList As New List(Of Database.DBElement)
        ...
        Dim BannerAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_MovieSet_Banner AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_MovieSet(Enums.ScrapeModifierType.MainBanner)
        Dim ClearArtAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_MovieSet_ClearArt AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_MovieSet(Enums.ScrapeModifierType.MainClearArt)
        Dim ClearLogoAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_MovieSet_ClearLogo AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_MovieSet(Enums.ScrapeModifierType.MainClearLogo)
        Dim DiscArtAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_MovieSet_DiscArt AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_MovieSet(Enums.ScrapeModifierType.MainDiscArt)
        Dim FanartAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_MovieSet_Fanart AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_MovieSet(Enums.ScrapeModifierType.MainFanart)
        Dim LandscapeAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_MovieSet_Landscape AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_MovieSet(Enums.ScrapeModifierType.MainLandscape)
        Dim PosterAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_MovieSet_Poster AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Image_MovieSet(Enums.ScrapeModifierType.MainPoster)
        ...
        For Each drvRow As DataRow In DataRowList
            ...
            Select Case tScrapeType
                ...
                Case Enums.ScrapeType.MissingAsk, Enums.ScrapeType.MissingAuto, Enums.ScrapeType.MissingSkip
                    ...
                    If Not String.IsNullOrEmpty(drvRow.Item("PosterPath").ToString) Then sModifier.MainPoster = False
            End Select
            ScrapeList.Add(New Database.DBElement(Enums.ContentType.MovieSet) With {
                           .ID = CLng(drvRow.Item("idMovieSet")),
                           .ScrapeModifiers = sModifier,
                           .ScrapeType = tScrapeType,
                           .ScrapeOptions = tScrapeOptions})
        Next
        ...
        If Not ScrapeList.Count = 0 Then
            ...
            bwMovieSetScraper.RunWorkerAsync(New Arguments With {.ScrapeList = ScrapeList})
        End If
    End Sub
    ...
    Private Sub InfoDownloaded_TV(ByRef DBTVShow As Database.DBElement)
        If DBTVShow.MainDetails.TitleSpecified Then
            ...
            Edit_TVShow(DBTVShow, Enums.AddonEventType.DuringScrapingSingle_TVShow)
        End If
    ...
    Private Sub CreateScrapeList_TV(ByVal tScrapeType As Enums.ScrapeType, ByVal tScrapeOptions As Structures.ScrapeOptions, ByVal tScrapeModifiers As Structures.ScrapeModifiers)
        Dim DataRowList As New List(Of DataRow)
        Dim ScrapeList As New List(Of Database.DBElement)
        ...
        Dim MainThemeAllowed As Boolean = Master.eSettings.FilenameAnyEnabled_TVShow_Theme AndAlso AddonsManager.Instance.ScraperWithCapabilityAnyEnabled_Theme_TV(Enums.ScrapeModifierType.MainTheme)
        ...
        For Each drvRow As DataRow In DataRowList
            ...
            Dim sModifier As New Structures.ScrapeModifiers
            sModifier.DoSearch = tScrapeModifiers.DoSearch
            sModifier.AllSeasonsBanner = tScrapeModifiers.AllSeasonsBanner AndAlso AllSeasonsBannerAllowed
            sModifier.AllSeasonsFanart = tScrapeModifiers.AllSeasonsFanart AndAlso AllSeasonsFanartAllowed
            sModifier.AllSeasonsLandscape = tScrapeModifiers.AllSeasonsLandscape AndAlso AllSeasonsLandscapeAllowed
            sModifier.AllSeasonsPoster = tScrapeModifiers.AllSeasonsPoster AndAlso AllSeasonsPosterAllowed
            sModifier.EpisodeActorThumbs = tScrapeModifiers.EpisodeActorThumbs AndAlso EpisodeActorThumbsAllowed
            sModifier.EpisodeFanart = tScrapeModifiers.EpisodeFanart AndAlso EpisodeFanartAllowed
            sModifier.EpisodeMeta = tScrapeModifiers.EpisodeMeta AndAlso EpisodeMetaAllowed
            sModifier.EpisodeNFO = tScrapeModifiers.EpisodeNFO
            sModifier.EpisodePoster = tScrapeModifiers.EpisodePoster AndAlso EpisodePosterAllowed
            sModifier.MainActorthumbs = tScrapeModifiers.MainActorthumbs AndAlso MainActorThumbsAllowed
            sModifier.MainBanner = tScrapeModifiers.MainBanner AndAlso MainBannerAllowed
            sModifier.MainCharacterArt = tScrapeModifiers.MainCharacterArt AndAlso MainCharacterArtAllowed
            sModifier.MainClearArt = tScrapeModifiers.MainClearArt AndAlso MainClearArtAllowed
            sModifier.MainClearLogo = tScrapeModifiers.MainClearLogo AndAlso MainClearLogoAllowed
            sModifier.MainExtrafanarts = tScrapeModifiers.MainExtrafanarts AndAlso MainExtrafanartsAllowed
            sModifier.MainFanart = tScrapeModifiers.MainFanart AndAlso MainFanartAllowed
            sModifier.MainLandscape = tScrapeModifiers.MainLandscape AndAlso MainLandscapeAllowed
            sModifier.MainNFO = tScrapeModifiers.MainNFO
            sModifier.MainPoster = tScrapeModifiers.MainPoster AndAlso MainPosterAllowed
            sModifier.MainTheme = tScrapeModifiers.MainTheme AndAlso MainThemeAllowed
            sModifier.SeasonBanner = tScrapeModifiers.SeasonBanner AndAlso SeasonBannerAllowed
            sModifier.SeasonFanart = tScrapeModifiers.SeasonFanart AndAlso SeasonFanartAllowed
            sModifier.SeasonLandscape = tScrapeModifiers.SeasonLandscape AndAlso SeasonLandscapeAllowed
            sModifier.SeasonPoster = tScrapeModifiers.SeasonPoster AndAlso SeasonPosterAllowed
            sModifier.withEpisodes = tScrapeModifiers.withEpisodes
            sModifier.withSeasons = tScrapeModifiers.withSeasons
            ...
            Select Case tScrapeType
                ...
                Case Enums.ScrapeType.MissingAsk, Enums.ScrapeType.MissingAuto, Enums.ScrapeType.MissingSkip
                    ...
                    If Not String.IsNullOrEmpty(drvRow.Item("ThemePath").ToString) Then sModifier.MainTheme = False
            End Select
            ScrapeList.Add(New Database.DBElement(Enums.ContentType.TVShow) With {
                           .ID = CLng(drvRow.Item("idShow")),
                           .ScrapeModifiers = sModifier,
                           .ScrapeType = tScrapeType,
                           .ScrapeOptions = tScrapeOptions})
        Next
        ...
        If Not ScrapeList.Count = 0 Then
            ...
            bwTVScraper.RunWorkerAsync(New Arguments With {.ScrapeList = ScrapeList})
        End If
    End Sub
    ...
    Private Sub InfoDownloaded_TVEpisode(ByRef DBTVEpisode As Database.DBElement)
        If Not String.IsNullOrEmpty(DBTVEpisode.MainDetails.Title) Then
            ...
            Edit_TVEpisode(DBTVEpisode, Enums.AddonEventType.DuringScrapingSingle_TVEpisode)
        End If
    ...
    Private Sub CreateScrapeList_TVEpisode(ByVal tScrapeType As Enums.ScrapeType, ByVal tScrapeOptions As Structures.ScrapeOptions, ByVal tScrapeModifiers As Structures.ScrapeModifiers)
        Dim DataRowList As New List(Of DataRow)
        Dim ScrapeList As New List(Of Database.DBElement)
        ...
        For Each drvRow As DataRow In DataRowList
            ...
            Dim sModifier As New Structures.ScrapeModifiers
            sModifier.DoSearch = tScrapeModifiers.DoSearch
            sModifier.EpisodeActorThumbs = tScrapeModifiers.EpisodeActorThumbs AndAlso ActorThumbsAllowed
            sModifier.EpisodeFanart = tScrapeModifiers.EpisodeFanart AndAlso FanartAllowed
            sModifier.EpisodeMeta = tScrapeModifiers.EpisodeMeta
            sModifier.EpisodeNFO = tScrapeModifiers.EpisodeNFO
            sModifier.EpisodePoster = tScrapeModifiers.EpisodePoster AndAlso PosterAllowed
            ...
            End Select
            ScrapeList.Add(New Database.DBElement(Enums.ContentType.TVEpisode) With {
                           .ID = CLng(drvRow.Item("idEpisode")),
                           .ScrapeModifiers = sModifier,
                           .ScrapeType = tScrapeType,
                           .ScrapeOptions = tScrapeOptions})
        Next
        ...
        If Not ScrapeList.Count = 0 Then
            ...
            bwTVEpisodeScraper.RunWorkerAsync(New Arguments With {.ScrapeList = ScrapeList})
        End If
    End Sub
    ...
    Private Sub InfoDownloaded_TVSeason(ByRef DBTVSeason As Database.DBElement)
        If Not String.IsNullOrEmpty(DBTVSeason.TVShowDetails.Title) Then
            ...
            Edit_TVSeason(DBTVSeason, Enums.AddonEventType.DuringScrapingSingle_TVSeason)
        End If
    ...
    Private Sub CreateScrapeList_TVSeason(ByVal tScrapeType As Enums.ScrapeType, ByVal tScrapeOptions As Structures.ScrapeOptions, ByVal tScrapeModifiers As Structures.ScrapeModifiers)
        Dim DataRowList As New List(Of DataRow)
        Dim ScrapeList As New List(Of Database.DBElement)
        ...
        If Not ScrapeList.Count = 0 Then
            ...
            bwTVSeasonScraper.RunWorkerAsync(New Arguments With {.ScrapeList = ScrapeList})
        End If
    End Sub
    ...
    Function MyResolveEventHandler(ByVal sender As Object, ByVal args As ResolveEventArgs) As [Assembly]
        ...
        Dim asm As Assembly = AddonsManager.AssemblyList.FirstOrDefault(Function(y) y.AssemblyName = name).Assembly
            ...
            If doOpen Then
                For Each sRow As DataGridViewRow In dgvMovies.SelectedRows
                    Using Explorer As New Process
                        ...
                        Dim strPath As String = FileUtils.Stacking.GetFirstStackedPath(sRow.Cells("MoviePath").Value.ToString)
                        ...
                        Else
                            Explorer.StartInfo.FileName = "xdg-open"
                            Explorer.StartInfo.Arguments = String.Format("""{0}""", Path.GetDirectoryName(strPath))
                        End If
                        Explorer.Start()
                    End Using
                Next
    ...
    Private Sub RunFilter_Movies(Optional ByVal doFill As Boolean = False)
        If Visible Then
            ...
            If FilterArray_Movies.Count > 0 Then
                ...
                AddonsManager.Instance.RuntimeObjects.FilterMovies = bsMovies.Filter
            Else
                If chkFilterDuplicates_Movies.Checked Then
                    lblFilter_Movies.Text = String.Format("{0} ({1})", Master.eLang.GetString(52, "Filters"), Master.eLang.GetString(1090, "Active"))
                Else
    ...
    Private Sub RunFilter_Shows(Optional ByVal doFill As Boolean = False)
        If Visible Then
            ...
            If FilterArray_TVShows.Count > 0 Then
                ...
                AddonsManager.Instance.RuntimeObjects.FilterTVShows = bsTVShows.Filter
            Else
                lblFilter_Shows.Text = String.Format("{0} ({1})", Master.eLang.GetString(52, "Filters"), Master.eLang.GetString(1091, "Inactive"))
            ...
            If doFill Then
                FillList(False, False, True)
                AddonsManager.Instance.RuntimeObjects.FilterTVShowsSearch = StringUtils.ConvertToValidFilterString(txtSearchShows.Text)
                AddonsManager.Instance.RuntimeObjects.FilterTVShowsType = cbSearchShows.Text
            Else
                txtSearchShows.Focus()
            End If
        End If
    ...
    Private Sub SelectRow_TVSeason(ByVal iRow As Integer)
        ...
        If dgvTVSeasons.Rows.Count > iRow Then
            If String.IsNullOrEmpty(dgvTVSeasons.Item("BannerPath", iRow).Value.ToString) AndAlso String.IsNullOrEmpty(dgvTVSeasons.Item("FanartPath", iRow).Value.ToString) AndAlso
                ...
                FillListTVEpisodes(Convert.ToInt64(dgvTVSeasons.Item("idShow", iRow).Value), Convert.ToInt32(dgvTVSeasons.Item("Season", iRow).Value))
            Else
    ...
    Private Sub SelectRow_TVShow(ByVal iRow As Integer)
        ...
        If dgvTVShows.Rows.Count > iRow Then
            If String.IsNullOrEmpty(dgvTVShows.Item("BannerPath", iRow).Value.ToString) AndAlso String.IsNullOrEmpty(dgvTVShows.Item("CharacterArtPath", iRow).Value.ToString) AndAlso
                ...
                FillListTVSeasons(Convert.ToInt64(dgvTVShows.Item("idShow", iRow).Value))
            Else
...
#Region "Fields"
        ...
        Dim Path As String
        Dim pURL As String
        Dim ScrapeList As List(Of Database.DBElement)
