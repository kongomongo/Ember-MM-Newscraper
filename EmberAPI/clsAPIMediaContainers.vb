        ...
        Public Sub SaveAllActorThumbs(ByRef DBElement As Database.DBElement)
            Select Case DBElement.ContentType
                Case Enums.ContentType.Movie
                    ...
                    Else
                        Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainActorThumbs, False)
...
#Region "Fields"
        ...
        Private _size As Enums.ImageSize
...
#Region "Properties"
        ...
        Public ReadOnly Property Size() As Enums.ImageSize
            Get
                Return _size
            End Get
        End Property
...
#Region "Methods"
        ...
        Private Sub DetectImageSize(ByRef strHeigth As String)
            Select Case strHeigth
                Case "3000"
                    _size = Enums.ImageSize.UHD3000
                Case "2160"
                    _size = Enums.ImageSize.UHD2160
                Case "2100"
                    _size = Enums.ImageSize.UHD2100
                Case "1500"
                    _size = Enums.ImageSize.HD1500
                Case "1440"
                    _size = Enums.ImageSize.QHD1440
                Case "1426"
                    _size = Enums.ImageSize.HD1426
                Case "1080"
                    _size = Enums.ImageSize.HD1080
                Case "1000"
                    _size = Enums.ImageSize.HD1000
                Case "720"
                    _size = Enums.ImageSize.HD720
                Case "578"
                    _size = Enums.ImageSize.HD578
                Case "562"
                    _size = Enums.ImageSize.HD562
                Case "512"
                    _size = Enums.ImageSize.HD512
                Case "310"
                    _size = Enums.ImageSize.HD310
                Case "281"
                    _size = Enums.ImageSize.SD281
                Case "225", "300" '225 = 16:9 / 300 = 4:3
                    _size = Enums.ImageSize.SD225
                Case "185"
                    _size = Enums.ImageSize.HD185
                Case "155"
                    _size = Enums.ImageSize.SD155
                Case "140"
                    _size = Enums.ImageSize.HD140
                Case Else
                    _size = Enums.ImageSize.Any
            End Select
        End Sub
            ...
            Select Case tContentType
                Case Enums.ContentType.Movie
                    ...
                    If Banner.LoadAndCache(tContentType, True) Then
                        If ForceFileCleanup Then Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainBanner, ForceFileCleanup)
                        Banner.LocalFilePath = Banner.ImageOriginal.Save_Movie(DBElement, Enums.ScrapeModifierType.MainBanner)
                    Else
                        Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainBanner, ForceFileCleanup)
                        Banner = New Image
                    End If

                    'Movie ClearArt
                    If ClearArt.LoadAndCache(tContentType, True) Then
                        If ForceFileCleanup Then Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainClearArt, ForceFileCleanup)
                        ClearArt.LocalFilePath = ClearArt.ImageOriginal.Save_Movie(DBElement, Enums.ScrapeModifierType.MainClearArt)
                    Else
                        Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainClearArt, ForceFileCleanup)
                        ClearArt = New Image
                    End If

                    'Movie ClearLogo
                    If ClearLogo.LoadAndCache(tContentType, True) Then
                        If ForceFileCleanup Then Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainClearLogo, ForceFileCleanup)
                        ClearLogo.LocalFilePath = ClearLogo.ImageOriginal.Save_Movie(DBElement, Enums.ScrapeModifierType.MainClearLogo)
                    Else
                        Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainClearLogo, ForceFileCleanup)
                        ClearLogo = New Image
                    End If

                    'Movie DiscArt
                    If DiscArt.LoadAndCache(tContentType, True) Then
                        If ForceFileCleanup Then Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainDiscArt, ForceFileCleanup)
                        DiscArt.LocalFilePath = DiscArt.ImageOriginal.Save_Movie(DBElement, Enums.ScrapeModifierType.MainDiscArt)
                    Else
                        Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainDiscArt, ForceFileCleanup)
                        DiscArt = New Image
                    End If
                    ...
                    Else
                        Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainExtrafanarts, ForceFileCleanup)
                    ...
                    Else
                        Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainExtrathumbs, ForceFileCleanup)
                    ...
                    If Fanart.LoadAndCache(tContentType, True) Then
                        If ForceFileCleanup Then Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainFanart, ForceFileCleanup)
                        Fanart.LocalFilePath = Fanart.ImageOriginal.Save_Movie(DBElement, Enums.ScrapeModifierType.MainFanart)
                    Else
                        Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainFanart, ForceFileCleanup)
                        Fanart = New Image
                    End If

                    'Movie Landscape
                    If Landscape.LoadAndCache(tContentType, True) Then
                        If ForceFileCleanup Then Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainLandscape, ForceFileCleanup)
                        Landscape.LocalFilePath = Landscape.ImageOriginal.Save_Movie(DBElement, Enums.ScrapeModifierType.MainLandscape)
                    Else
                        Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainLandscape, ForceFileCleanup)
                        Landscape = New Image
                    End If

                    'Movie Poster
                    If Poster.LoadAndCache(tContentType, True) Then
                        If ForceFileCleanup Then Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainPoster, ForceFileCleanup)
                        Poster.LocalFilePath = Poster.ImageOriginal.Save_Movie(DBElement, Enums.ScrapeModifierType.MainPoster)
                    Else
                        Images.Delete_Movie(DBElement, Enums.ScrapeModifierType.MainPoster, ForceFileCleanup)
                        Poster = New Image
                    End If
                ...
                Case Enums.ContentType.MovieSet
                    ...
                    'MovieSet Banner
                    If Banner.LoadAndCache(tContentType, True) Then
                        If DBElement.MainDetails.TitleHasChanged Then Images.Delete_MovieSet(DBElement, Enums.ScrapeModifierType.MainBanner, True)
                        Banner.LocalFilePath = Banner.ImageOriginal.Save_MovieSet(DBElement, Enums.ScrapeModifierType.MainBanner)
                    Else
                        Images.Delete_MovieSet(DBElement, Enums.ScrapeModifierType.MainBanner, DBElement.MainDetails.TitleHasChanged)
                        Banner = New Image
                    End If

                    'MovieSet ClearArt
                    If ClearArt.LoadAndCache(tContentType, True) Then
                        If DBElement.MainDetails.TitleHasChanged Then Images.Delete_MovieSet(DBElement, Enums.ScrapeModifierType.MainClearArt, True)
                        ClearArt.LocalFilePath = ClearArt.ImageOriginal.Save_MovieSet(DBElement, Enums.ScrapeModifierType.MainClearArt)
                    Else
                        Images.Delete_MovieSet(DBElement, Enums.ScrapeModifierType.MainClearArt, DBElement.MainDetails.TitleHasChanged)
                        ClearArt = New Image
                    End If

                    'MovieSet ClearLogo
                    If ClearLogo.LoadAndCache(tContentType, True) Then
                        If DBElement.MainDetails.TitleHasChanged Then Images.Delete_MovieSet(DBElement, Enums.ScrapeModifierType.MainClearLogo, True)
                        ClearLogo.LocalFilePath = ClearLogo.ImageOriginal.Save_MovieSet(DBElement, Enums.ScrapeModifierType.MainClearLogo)
                    Else
                        Images.Delete_MovieSet(DBElement, Enums.ScrapeModifierType.MainClearLogo, DBElement.MainDetails.TitleHasChanged)
                        ClearLogo = New Image
                    End If

                    'MovieSet DiscArt
                    If DiscArt.LoadAndCache(tContentType, True) Then
                        If DBElement.MainDetails.TitleHasChanged Then Images.Delete_MovieSet(DBElement, Enums.ScrapeModifierType.MainDiscArt, True)
                        DiscArt.LocalFilePath = DiscArt.ImageOriginal.Save_MovieSet(DBElement, Enums.ScrapeModifierType.MainDiscArt)
                    Else
                        Images.Delete_MovieSet(DBElement, Enums.ScrapeModifierType.MainDiscArt, DBElement.MainDetails.TitleHasChanged)
                        DiscArt = New Image
                    End If

                    'MovieSet Fanart
                    If Fanart.LoadAndCache(tContentType, True) Then
                        If DBElement.MainDetails.TitleHasChanged Then Images.Delete_MovieSet(DBElement, Enums.ScrapeModifierType.MainFanart, True)
                        Fanart.LocalFilePath = Fanart.ImageOriginal.Save_MovieSet(DBElement, Enums.ScrapeModifierType.MainFanart)
                    Else
                        Images.Delete_MovieSet(DBElement, Enums.ScrapeModifierType.MainFanart, DBElement.MainDetails.TitleHasChanged)
                        Fanart = New Image
                    End If

                    'MovieSet Landscape
                    If Landscape.LoadAndCache(tContentType, True) Then
                        If DBElement.MainDetails.TitleHasChanged Then Images.Delete_MovieSet(DBElement, Enums.ScrapeModifierType.MainLandscape, True)
                        Landscape.LocalFilePath = Landscape.ImageOriginal.Save_MovieSet(DBElement, Enums.ScrapeModifierType.MainLandscape)
                    Else
                        Images.Delete_MovieSet(DBElement, Enums.ScrapeModifierType.MainLandscape, DBElement.MainDetails.TitleHasChanged)
                        Landscape = New Image
                    End If

                    'MovieSet Poster
                    If Poster.LoadAndCache(tContentType, True) Then
                        If DBElement.MainDetails.TitleHasChanged Then Images.Delete_MovieSet(DBElement, Enums.ScrapeModifierType.MainPoster, True)
                        Poster.LocalFilePath = Poster.ImageOriginal.Save_MovieSet(DBElement, Enums.ScrapeModifierType.MainPoster)
                    Else
                        Images.Delete_MovieSet(DBElement, Enums.ScrapeModifierType.MainPoster, DBElement.MainDetails.TitleHasChanged)
                        Poster = New Image
                    End If
                ...
                Case Enums.ContentType.TVEpisode
                    ...
                    'Episode Fanart
                    If Fanart.LoadAndCache(tContentType, True) Then
                        Fanart.LocalFilePath = Fanart.ImageOriginal.Save_TVEpisode(DBElement, Enums.ScrapeModifierType.EpisodeFanart)
                    Else
                        Images.Delete_TVEpisode(DBElement, Enums.ScrapeModifierType.EpisodeFanart)
                        Fanart = New Image
                    End If

                    'Episode Poster
                    If Poster.LoadAndCache(tContentType, True) Then
                        Poster.LocalFilePath = Poster.ImageOriginal.Save_TVEpisode(DBElement, Enums.ScrapeModifierType.EpisodePoster)
                    Else
                        Images.Delete_TVEpisode(DBElement, Enums.ScrapeModifierType.EpisodePoster)
                        Poster = New Image
                    End If
                ...
                Case Enums.ContentType.TVSeason
                    ...
                    If Banner.LoadAndCache(tContentType, True) Then
                        If DBElement.MainDetails.Season = 999 Then
                            Banner.LocalFilePath = Banner.ImageOriginal.Save_TVAllSeasons(DBElement, Enums.ScrapeModifierType.AllSeasonsBanner)
                        Else
                            Banner.LocalFilePath = Banner.ImageOriginal.Save_TVSeason(DBElement, Enums.ScrapeModifierType.SeasonBanner)
                        End If
                    Else
                        If DBElement.MainDetails.Season = 999 Then
                            Images.Delete_TVAllSeasons(DBElement, Enums.ScrapeModifierType.AllSeasonsBanner)
                            Banner = New Image
                        Else
                            Images.Delete_TVSeason(DBElement, Enums.ScrapeModifierType.SeasonBanner)
                            Banner = New Image
                        End If
                    End If
                    ...
                    If Fanart.LoadAndCache(tContentType, True) Then
                        If DBElement.MainDetails.Season = 999 Then
                            Fanart.LocalFilePath = Fanart.ImageOriginal.Save_TVAllSeasons(DBElement, Enums.ScrapeModifierType.AllSeasonsFanart)
                        Else
                            Fanart.LocalFilePath = Fanart.ImageOriginal.Save_TVSeason(DBElement, Enums.ScrapeModifierType.SeasonFanart)
                        End If
                    Else
                        If DBElement.MainDetails.Season = 999 Then
                            Images.Delete_TVAllSeasons(DBElement, Enums.ScrapeModifierType.AllSeasonsFanart)
                            Fanart = New Image
                        Else
                            Images.Delete_TVSeason(DBElement, Enums.ScrapeModifierType.SeasonFanart)
                            Fanart = New Image
                        End If
                    End If
                    ...
                    If Landscape.LoadAndCache(tContentType, True) Then
                        If DBElement.MainDetails.Season = 999 Then
                            Landscape.LocalFilePath = Landscape.ImageOriginal.Save_TVAllSeasons(DBElement, Enums.ScrapeModifierType.AllSeasonsLandscape)
                        Else
                            Landscape.LocalFilePath = Landscape.ImageOriginal.Save_TVSeason(DBElement, Enums.ScrapeModifierType.SeasonLandscape)
                        End If
                    Else
                        If DBElement.MainDetails.Season = 999 Then
                            Images.Delete_TVAllSeasons(DBElement, Enums.ScrapeModifierType.AllSeasonsLandscape)
                            Landscape = New Image
                        Else
                            Images.Delete_TVSeason(DBElement, Enums.ScrapeModifierType.SeasonLandscape)
                            Landscape = New Image
                        End If
                    End If
                    ...
                    If Poster.LoadAndCache(tContentType, True) Then
                        If DBElement.MainDetails.Season = 999 Then
                            Poster.LocalFilePath = Poster.ImageOriginal.Save_TVAllSeasons(DBElement, Enums.ScrapeModifierType.AllSeasonsPoster)
                        Else
                            Poster.LocalFilePath = Poster.ImageOriginal.Save_TVSeason(DBElement, Enums.ScrapeModifierType.SeasonPoster)
                        End If
                    Else
                        If DBElement.MainDetails.Season = 999 Then
                            Images.Delete_TVAllSeasons(DBElement, Enums.ScrapeModifierType.AllSeasonsPoster)
                            Poster = New Image
                        Else
                            Images.Delete_TVSeason(DBElement, Enums.ScrapeModifierType.SeasonPoster)
                            Poster = New Image
                        End If
                    End If
                ...
                Case Enums.ContentType.TVShow
                    ...
                    'Show Banner
                    If Banner.LoadAndCache(tContentType, True) Then
                        Banner.LocalFilePath = Banner.ImageOriginal.Save_TVShow(DBElement, Enums.ScrapeModifierType.MainBanner)
                    Else
                        Images.Delete_TVShow(DBElement, Enums.ScrapeModifierType.MainBanner)
                        Banner = New Image
                    End If

                    'Show CharacterArt
                    If CharacterArt.LoadAndCache(tContentType, True) Then
                        CharacterArt.LocalFilePath = CharacterArt.ImageOriginal.Save_TVShow(DBElement, Enums.ScrapeModifierType.MainCharacterArt)
                    Else
                        Images.Delete_TVShow(DBElement, Enums.ScrapeModifierType.MainCharacterArt)
                        CharacterArt = New Image
                    End If

                    'Show ClearArt
                    If ClearArt.LoadAndCache(tContentType, True) Then
                        ClearArt.LocalFilePath = ClearArt.ImageOriginal.Save_TVShow(DBElement, Enums.ScrapeModifierType.MainClearArt)
                    Else
                        Images.Delete_TVShow(DBElement, Enums.ScrapeModifierType.MainClearArt)
                        ClearArt = New Image
                    End If

                    'Show ClearLogo
                    If ClearLogo.LoadAndCache(tContentType, True) Then
                        ClearLogo.LocalFilePath = ClearLogo.ImageOriginal.Save_TVShow(DBElement, Enums.ScrapeModifierType.MainClearLogo)
                    Else
                        Images.Delete_TVShow(DBElement, Enums.ScrapeModifierType.MainClearLogo)
                        ClearLogo = New Image
                    End If
                    ...
                    Else
                        Images.Delete_TVShow(DBElement, Enums.ScrapeModifierType.MainExtrafanarts)
                        Extrafanarts = New List(Of Image)
                        DBElement.ExtrafanartsPath = String.Empty
                    End If
                    ...
                    'Show Fanart
                    If Fanart.LoadAndCache(tContentType, True) Then
                        Fanart.LocalFilePath = Fanart.ImageOriginal.Save_TVShow(DBElement, Enums.ScrapeModifierType.MainFanart)
                    Else
                        Images.Delete_TVShow(DBElement, Enums.ScrapeModifierType.MainFanart)
                        Fanart = New Image
                    End If

                    'Show Landscape
                    If Landscape.LoadAndCache(tContentType, True) Then
                        Landscape.LocalFilePath = Landscape.ImageOriginal.Save_TVShow(DBElement, Enums.ScrapeModifierType.MainLandscape)
                    Else
                        Images.Delete_TVShow(DBElement, Enums.ScrapeModifierType.MainLandscape)
                        Landscape = New Image
                    End If

                    'Show Poster
                    If Poster.LoadAndCache(tContentType, True) Then
                        Poster.LocalFilePath = Poster.ImageOriginal.Save_TVShow(DBElement, Enums.ScrapeModifierType.MainPoster)
                    Else
                        Images.Delete_TVShow(DBElement, Enums.ScrapeModifierType.MainPoster)
                        Poster = New Image
                    End If
            End Select
        End Sub
...
#Region "Methods"
            ...
            Select Case tDBElement.ContentType
                Case Enums.ContentType.Movie
                    With Master.eSettings.Movie.ImageSettings
                        cSettings.ForcedLanguage = .ImagesForcedLanguage
                        cSettings.ForceLanguage = .ImagesForceLanguage
                        cSettings.GetBlankImages = .ImagesGetBlankImages
                        cSettings.GetEnglishImages = .ImagesGetEnglishImages
                        cSettings.MediaLanguageOnly = .ImagesMediaLanguageOnly
                    End With
                Case Enums.ContentType.MovieSet
        ...
        Private Sub SortImages(ByVal cSettings As FilterSettings)
            Select Case cSettings.ContentType
                Case Enums.ContentType.Movie
                    'Movie Banner
                    If Not Master.eSettings.Movie.ImageSettings.BannerPrefSize = Enums.ImageSize.Any Then
                        _mainbanners = _mainbanners.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.Movie.ImageSettings.BannerPrefSize).ToList()
                    Else
                        _mainbanners = _mainbanners.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
                    End If
                    'Movie ClearArt
                    ...
                    'Movie Fanart
                    If Not Master.eSettings.Movie.ImageSettings.FanartPrefSize = Enums.ImageSize.Any Then
                        _mainfanarts = _mainfanarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.Movie.ImageSettings.FanartPrefSize).ToList()
                    Else
                        _mainfanarts = _mainfanarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
                    End If
                    'Movie Landscape
                    _mainlandscapes = _mainlandscapes.OrderByDescending(Function(z) z.VoteAverage).ToList()
                    'Movie Poster
                    If Not Master.eSettings.Movie.ImageSettings.PosterPrefSize = Enums.ImageSize.Any Then
                        _mainposters = _mainposters.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.Movie.ImageSettings.PosterPrefSize).ToList()
                    Else
                        _mainposters = _mainposters.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
                    End If
                Case Enums.ContentType.MovieSet
                    'MovieSet Banner
                    If Not Master.eSettings.Movieset.ImageSettings.BannerPrefSize = Enums.ImageSize.Any Then
                        _mainbanners = _mainbanners.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.Movieset.ImageSettings.BannerPrefSize).ToList()
                    Else
                        _mainbanners = _mainbanners.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
                    End If
                    'MovieSet ClearArt
                    ...
                    _maindiscarts = _maindiscarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.DiscType).ToList()
                    'MovieSet Fanart
                    If Not Master.eSettings.Movieset.ImageSettings.FanartPrefSize = Enums.ImageSize.Any Then
                        _mainfanarts = _mainfanarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.Movieset.ImageSettings.FanartPrefSize).ToList()
                    Else
                        _mainfanarts = _mainfanarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
                    End If
                    'MovieSet Landscape
                    _mainlandscapes = _mainlandscapes.OrderByDescending(Function(z) z.VoteAverage).ToList()
                    'MovieSet Poster
                    If Not Master.eSettings.Movieset.ImageSettings.PosterPrefSize = Enums.ImageSize.Any Then
                        _mainposters = _mainposters.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.Movieset.ImageSettings.PosterPrefSize).ToList()
                    Else
                        _mainposters = _mainposters.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
                    End If
                Case Enums.ContentType.TV, Enums.ContentType.TVShow
                    ...
                    If Not Master.eSettings.TVShowBannerPrefSize = Enums.ImageSize.Any Then
                        _mainbanners = _mainbanners.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.TVShowBannerPrefSize).ToList()
                    Else
                        _mainbanners = _mainbanners.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
                    End If
                    'TVShow CharacterArt
                    ...
                    If Not Master.eSettings.TVShowFanartPrefSize = Enums.ImageSize.Any Then
                        _mainfanarts = _mainfanarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.TVShowFanartPrefSize).ToList()
                    Else
                        _mainfanarts = _mainfanarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
                    End If
                    'TVShow Landscape
                    ...
                    If Not Master.eSettings.TVShowPosterPrefSize = Enums.ImageSize.Any Then
                        _mainposters = _mainposters.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.TVShowPosterPrefSize).ToList()
                    Else
                        _mainposters = _mainposters.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
                    End If
                Case Enums.ContentType.TVEpisode
                    ...
                    If Not Master.eSettings.TVEpisodeFanartPrefSize = Enums.ImageSize.Any Then
                        _mainfanarts = _mainfanarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.TVEpisodeFanartPrefSize).ToList()
                    Else
                        _mainfanarts = _mainfanarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
                    End If
                Case Enums.ContentType.TVSeason
                    ...
                    If Not Master.eSettings.TVSeasonFanartPrefSize = Enums.ImageSize.Any Then
                        _mainfanarts = _mainfanarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.TVSeasonFanartPrefSize).ToList()
                    Else
                        _mainfanarts = _mainfanarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
                    End If
                    'TVShow Poster (TVSeason preferred sorting)
                    If Not Master.eSettings.TVSeasonPosterPrefSize = Enums.ImageSize.Any Then
                        _mainposters = _mainposters.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.TVSeasonPosterPrefSize).ToList()
                    Else
                        _mainposters = _mainposters.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
                    End If
            End Select
            ...
            'TVEpisode Fanart
            If Not Master.eSettings.TVEpisodeFanartPrefSize = Enums.ImageSize.Any Then
                _episodefanarts = _episodefanarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.TVEpisodeFanartPrefSize).ToList()
            Else
                _episodefanarts = _episodefanarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
            End If
            'TVEpisode Poster
            If Not Master.eSettings.TVEpisodePosterPrefSize = Enums.ImageSize.Any Then
                _episodeposters = _episodeposters.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.TVEpisodePosterPrefSize).ToList()
            Else
                _episodeposters = _episodeposters.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
            End If
            'TVSeason Banner
            If Not Master.eSettings.TVSeasonBannerPrefSize = Enums.ImageSize.Any Then
                _seasonbanners = _seasonbanners.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.TVSeasonBannerPrefSize).ToList()
            Else
                _seasonbanners = _seasonbanners.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
            End If
            'TVSeason Fanart
            If Not Master.eSettings.TVSeasonFanartPrefSize = Enums.ImageSize.Any Then
                _seasonfanarts = _seasonfanarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.TVSeasonFanartPrefSize).ToList()
            Else
                _seasonfanarts = _seasonfanarts.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
            End If
            'TVSeason Landscape
            _seasonlandscapes = _seasonlandscapes.OrderByDescending(Function(z) z.VoteAverage).ToList()
            'TVSeason Poster
            If Not Master.eSettings.TVSeasonPosterPrefSize = Enums.ImageSize.Any Then
                _seasonposters = _seasonposters.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).OrderByDescending(Function(y) y.Size = Master.eSettings.TVSeasonPosterPrefSize).ToList()
            Else
                _seasonposters = _seasonposters.OrderByDescending(Function(z) z.VoteAverage).OrderBy(Function(x) x.Size).ToList()
            End If
        End Sub
