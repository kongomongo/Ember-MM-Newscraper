...
#Region "Fields"
    ...
    'Movie
    Private _RatedMovies As IEnumerable(Of Objects.Get.Ratings.TraktRatingsItem)
    Private _WatchedMovies As IEnumerable(Of Objects.Get.Watched.TraktWatchedMovie)
    Private _WatchedAndRatedMovies As New List(Of clsAPITrakt.WatchedAndRatedMovie)

    'TVShow
    Private _RatedTVShows As IEnumerable(Of Objects.Get.Ratings.TraktRatingsItem)
    Private _WatchedTVShows As IEnumerable(Of Objects.Get.Watched.TraktWatchedShow)
    Private _WatchedAndRatedTVShows As New List(Of clsAPITrakt.WatchedAndRatedTVShow)
    ...
    'Private traktLists As New List(Of Objects.Get.SyncTraktAPI.Model.TraktSyncList)
...
#Region "Methods"
    ...
    Sub SetUp()
        Try
            ...
            tbTrakt.Enabled = _TraktAPI.CheckConnection
            ...
            prgPlaycount.Maximum = _WatchedAndRatedMovies.Count
            ...
            prgtraktWatchlist.Maximum = _WatchedAndRatedMovies.Count
        ...
        Try
            If e.RowIndex > -1 Then
                If dgvtraktWatchlist(e.ColumnIndex, e.RowIndex).Value.ToString().StartsWith("http") Then
                    Process.Start(dgvtraktWatchlist.CurrentCell.Value.ToString())
                End If
            End If
        Catch ex As Exception
            logger.Error(ex, New StackFrame().GetMethod().Name)
...
#Region "Trakt.tv Sync Playcount"
    ...
    Private Sub btnPlaycountGetList_Movies_Movies_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPlaycountGetList_Movies.Click
        ...
        _RatedMovies = Nothing
        _RatedTVShows = Nothing
        _WatchedMovies = Nothing
        _WatchedTVShows = Nothing
        _WatchedAndRatedMovies = Nothing
        _WatchedAndRatedTVShows = Nothing

        dgvPlaycount.DataSource = Nothing
        dgvPlaycount.Rows.Clear()
        btnPlaycountSyncWatched_TVShows.Enabled = False

        _WatchedMovies = _TraktAPI.GetWatched_Movies()
        _RatedMovies = _TraktAPI.GetRated_Movies()

        If _WatchedMovies Is Nothing Then
            dgvPlaycount.DataSource = Nothing
            dgvPlaycount.Rows.Clear()
            ...
            Exit Sub
        Else
            _WatchedAndRatedMovies = _TraktAPI.GetWatchedAndRated_Movies(_WatchedMovies, _RatedMovies)
            ...
            'fill rows
            For Each nMovie In _WatchedAndRatedMovies
                dgvPlaycount.Rows.Add(New Object() {nMovie.Movie.Ids.Trakt, nMovie.Movie.Title, nMovie.Plays, nMovie.LastWatchedAt, String.Empty, nMovie.Rating})
            Next
    ...
    Private Sub btnPlaycountGetList_TVShows_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPlaycountGetList_TVShows.Click
        ...
        _RatedTVShows = _TraktAPI.GetRated_TVShows()
        _WatchedTVShows = _TraktAPI.GetWatched_TVShows(True)

        If _WatchedTVShows Is Nothing Then
            dgvPlaycount.DataSource = Nothing
            dgvPlaycount.Rows.Clear()
            btnSaveWatchedStateToEmber.Enabled = False
            btnPlaycountSyncWatched_Movies.Enabled = True
            logger.Info("No watched tv shows scraped from trakt.tv!")
            Exit Sub
        Else
            _WatchedAndRatedTVShows = _TraktAPI.GetWatchedAndRated_TVShows(_WatchedTVShows, _RatedTVShows)
            dgvPlaycount.AutoGenerateColumns = True
            btnSaveWatchedStateToEmber.Enabled = True
            ...
            dgvPlaycount.AutoGenerateColumns = False
            'fill rows
            For Each nTVShow In _WatchedAndRatedTVShows
                dgvPlaycount.Rows.Add(New Object() {nTVShow.Show.Ids.Trakt, nTVShow.Show.Title, nTVShow.WatchedEpisodes, nTVShow.LastWatchedAt, String.Format("{0} / {1}", nTVShow.WatchedEpisodes, nTVShow.Show.AiredEpisodes), String.Empty})
            Next
            Cursor = Cursors.Default
            dgvPlaycount.Sort(colPlaycountLastWatched, System.ComponentModel.ListSortDirection.Descending)
        End If
    ...
    Private Sub btnPlaycountSyncRating_Click(sender As Object, e As EventArgs) Handles btnPlaycountSyncRating.Click
        ...
        '    For Each tMovie In _WatchedAndRatedMovies.Where(Function(f) f.Modified)
    ...
    Private Sub btnPlaycountSyncDeleteItem_Click(sender As Object, e As EventArgs) Handles btnPlaycountSyncDeleteItem.Click
        ...
        'If _WatchedAndRatedMovies Is Nothing Then
        '    IsMovie = False
        'End If
        ...
        'If IsMovie Then
        '    If _WatchedAndRatedMovies IsNot Nothing Then
        '        'search by TraktID
        ...
        '            Dim nMovie = _WatchedAndRatedMovies.FirstOrDefault(Function(f) f.Movie.Ids.Trakt IsNot Nothing AndAlso
    ...
    Private Sub btnPlaycountSyncWatched_Movies_Click(sender As Object, e As EventArgs) Handles btnPlaycountSyncWatched_Movies.Click
        ...
        '            SyncThisItem = True
        '            If _WatchedAndRatedMovies IsNot Nothing Then
        '                For Each watchedMovieData In _WatchedAndRatedMovies
    ...
    Private Sub btnSaveWatchedStateToEmber_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSaveWatchedStateToEmber.Click
        ...
        'If _WatchedMovies IsNot Nothing Then
        '    'save movie playcount!
        '    prgPlaycount.Value = 0
        '    prgPlaycount.Maximum = _WatchedMovies.Where(Function(f) f.Movie.Ids.Imdb IsNot Nothing OrElse
    ...
    Private Sub bwSaveWatchedStateToEmber_Movies_DoWork(ByVal sender As Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles bwSaveWatchedStateToEmber_Movies.DoWork
        _TraktAPI.SaveWatchedStateToEmber_Movies(_WatchedMovies, AddressOf ShowProgress_SaveWatchedStateToEmber_Movies)
    End Sub
    ...
    Private Sub dgvPlaycount_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvPlaycount.CellEndEdit
        'If e.RowIndex > -1 AndAlso e.ColumnIndex = 4 AndAlso dgvPlaycount.CurrentCell.RowIndex > -1 AndAlso _WatchedAndRatedMovies IsNot Nothing Then
...
#Region "Trakt.tv Comments"
    ...
    Private Sub dgvtraktComments_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvtraktComments.CellContentClick
        If e.RowIndex > -1 AndAlso e.ColumnIndex = 4 AndAlso dgvtraktComments.CurrentCell.RowIndex > -1 Then
            ...
            Process.Start("http://trakt.tv/comments/" & dgvtraktComments.CurrentCell.Value.ToString())
        End If
    End Sub
