                    ...
                    For Each _tag In globalMovieTags
                        If _tag.Name = lbTags.SelectedItem.ToString Then
                            If Not _tag.Movies Is Nothing Then
                                ...
                                For Each movie In _tag.Movies
                                    If movie.MainDetails.Title = tmpMovie.MainDetails.Title AndAlso movie.MainDetails.IMDB = tmpMovie.MainDetails.IMDB Then
                                        alreadyintag = True
                                    End If
                                Next
                                ...
                                Exit For
                            Else
                                logger.Info("[" & _tag.Name & "] " & tmpMovie.MainDetails.Title & " is null! Error when trying to add movie to tag!")
                            End If
                        End If
                    Next
    ...
    Private Sub SyncTagToMovies(ByVal tag As SyncTag)
        For Each tmovie As Database.DBElement In tag.Movies
            If tmovie.MainDetails.Title.EndsWith("_TODELETE") Then
                ...
                tmovie.MainDetails.Title = tmovie.MainDetails.Title.Replace("_TODELETE", "")
