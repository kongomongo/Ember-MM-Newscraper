    ...
    Public Function Scrape_Movie(ByVal strOriginalTitle As String, ByVal strIMDB As String, ByVal tScrapeModifiers As Structures.ScrapeModifiers, ByVal tScrapeOptions As Structures.ScrapeOptions) As Interfaces.AddonResult
        ...
        Try
            If Not String.IsNullOrEmpty(strOriginalTitle) Then
                ...
                'title to search for
                Dim searchtitle As String = strOriginalTitle
                ...
                If sjson = String.Empty OrElse sjson.Length < 50 Then
                    ...
                    logger.Debug("[" & strOriginalTitle & "] Apple Trailer - Movie NOT found on http://trailers.apple.com/ - URL: " & String.Format("http://trailers.apple.com/trailers/home/scripts/quickfind.php?q={0}", searchtitle))
                    ...
                    searchtitle = HttpUtility.UrlEncode(strOriginalTitle)
                    SearchURL = String.Concat(BaseURL, searchtitle)
                    ...
                    If Not String.IsNullOrEmpty(sHtml) Then
                        ...
                        If Not trailerWEBURLResults Is Nothing AndAlso trailerWEBURLResults.Count > 0 Then
                            ...
                            searchtitle = (StringUtils.RemovePunctuation(strOriginalTitle.ToLower)).Replace(" ", String.Empty)
                                ...
                                If Not String.IsNullOrEmpty(TrailerWEBPageURL) Then
                                    ...
                                    If TrailerWEBPageURL.Contains("/") Then
                                        ...
                                        If (strOriginalTitle.Length <= 5 AndAlso compareresult = 0) OrElse (strOriginalTitle.Length > 5 AndAlso compareresult <= 2) Then
                                            logger.Debug("[" & strOriginalTitle & "] Apple Trailer - Google: Movie found! URL: " & TrailerWEBPageURL)
                                            Exit For
                                        Else
                                            logger.Debug("[" & strOriginalTitle & "] Apple Trailer - Google: wrong result! URL: " & TrailerWEBPageURL)
                                            TrailerWEBPageURL = String.Empty
                                        End If
                                    End If
                                End If
                            Next
                        Else
                            logger.Debug("[" & strOriginalTitle & "] Apple Trailer - Movie NOT found on http://trailers.apple.com/ using google! URL: " & SearchURL)
                        End If
                    Else
                        logger.Debug("[" & strOriginalTitle & "] Apple Trailer - Movie NOT found on http://trailers.apple.com/ using google! URL: " & SearchURL)
                    End If
                ...
                Else
                    ...
                    If Not jsonresult Is Nothing AndAlso Not jsonresult.error AndAlso Not jsonresult.results Is Nothing AndAlso jsonresult.results.Count > 0 Then
                        ...
                        If String.IsNullOrEmpty(jsonresult.results(0).location) Then
                            logger.Debug("[" & strOriginalTitle & "] Apple Trailer - Movie NOT found on http://trailers.apple.com/ - URL: " & TrailerWEBPageURL)
                        Else
                            logger.Debug("[" & strOriginalTitle & "] Apple Trailer - Movie found on http://trailers.apple.com/ - URL: " & TrailerWEBPageURL)
                        End If
                    Else
                        logger.Debug("[" & strOriginalTitle & "] Apple Trailer - Movie NOT found on http://trailers.apple.com/ - URL: " & String.Format("http://trailers.apple.com/trailers/home/scripts/quickfind.php?q={0}", searchtitle))
                    End If
                ...
                If Not String.IsNullOrEmpty(TrailerWEBPageURL) Then
                    ...
                    logger.Info("[" & strOriginalTitle & "] Apple Trailer - Movie found! Download URL: " & TrailerWEBPageURL)
                    ...
                    prevQual = Master.eSettings.Movie.TrailerSettings.PrefVideoQuality.ToString

                    'Since prevQual value will be used to build trailerurl "Any" should not be used at all!
                    If prevQual.Contains("Any") Then
                        prevQual = Master.eSettings.Movie.TrailerSettings.MinVideoQuality.ToString
                        If prevQual.Contains("Any") Then
                            prevQual = "480p"
                        End If
                    End If
                    ...
                    If Not zResult Is Nothing AndAlso zResult.Count > 0 Then
                        For ctr As Integer = 0 To zResult.Count - 1
                            ...
                            nAddonResult.ScraperResult_Trailers.Add(New MediaContainers.Trailer With {.URLVideoStream = zResult.Item(ctr).Groups("LINK").Value, .Title = zResult.Item(ctr).Groups("TITLE").Value, .Duration = zResult.Item(ctr).Groups("DURATION").Value})
                        Next
                        ...
                        For Each nTrailer In nAddonResult.ScraperResult_Trailers
                            ...
                            Dim TrailerSiteLink As String = String.Concat(TrailerWEBPageURL, nTrailer.URLVideoStream)
                            ...
                            If yResult.Count > 0 Then
                                ...
                                nTrailer.URLWebsite = tDownloadURL
                                ...
                                nTrailer.URLVideoStream = tDownloadURL
                                nTrailer.Scraper = "Apple"
                                nTrailer.Source = "Apple"
                    ...
                    End If
                Else
                    logger.Info("[" & strOriginalTitle & "] Apple Trailer - Movie NOT found")
                End If
            Else
                logger.Warn("[" & strOriginalTitle & "] Apple Trailer - Original title is empty, no scraping of trailers possible!")
            End If

            'log scraperesults for user
            If nAddonResult.ScraperResult_Trailers.Count < 1 Then
                logger.Info("[" & strOriginalTitle & "] Apple Trailer - NO trailer scraped!")
            Else
                logger.Info("[" & strOriginalTitle & "] Apple Trailer - Scraped " & nAddonResult.ScraperResult_Trailers.Count & " trailer!")
            End If
