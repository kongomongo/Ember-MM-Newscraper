...
#Region "Methods"
    ...
    Public Function GetInfo_TVEpisode(ByVal iShowTVDB As Integer, ByVal iSeason As Integer, ByVal iEpisode As Integer, ByVal tEpisodeOrdering As Enums.EpisodeOrdering, ByVal tScrapeModifiers As Structures.ScrapeModifiers, ByVal tScrapeOptions As Structures.ScrapeOptions) As MediaContainers.MainDetails
        Try
            Dim APIResult As Task(Of TVDB.Model.SeriesDetails) = Task.Run(Function() GetFullSeriesById(CInt(iShowTVDB)))
            ...
            If Not nEpisodeInfo Is Nothing Then
                Dim nEpisode As MediaContainers.MainDetails = GetInfo_TVEpisode(nEpisodeInfo, nResult, tScrapeModifiers, tScrapeOptions)
                Return nEpisode
            Else
                Return Nothing
            End If
    ...
    Public Function GetInfo_TVEpisode(ByVal iShowTVDB As Integer, ByVal strAired As String, ByVal tScrapeModifiers As Structures.ScrapeModifiers, ByVal tScrapeOptions As Structures.ScrapeOptions) As MediaContainers.MainDetails
        Dim APIResult As Task(Of TVDB.Model.SeriesDetails) = Task.Run(Function() GetFullSeriesById(CInt(iShowTVDB)))
        If APIResult Is Nothing OrElse APIResult.Result Is Nothing Then
            Return Nothing
        End If
        ...
        If EpisodeList IsNot Nothing AndAlso EpisodeList.Count = 1 Then
            Dim nEpisode As MediaContainers.MainDetails = GetInfo_TVEpisode(EpisodeList(0), nResult, tScrapeModifiers, tScrapeOptions)
            Return nEpisode
        Else
            Return Nothing
        End If
    ...
    Public Function GetInfo_TVEpisode(ByRef tEpisodeInfo As TVDB.Model.Episode, ByRef tTVShowInfo As TVDB.Model.SeriesDetails, ByVal tScrapeModifiers As Structures.ScrapeModifiers, ByVal tScrapeOptions As Structures.ScrapeOptions) As MediaContainers.MainDetails
        ...
        'Season # Combined
        If Not CInt(tEpisodeInfo.CombinedSeason) = -1 Then
            nEpisode.SeasonCombined = tEpisodeInfo.CombinedSeason
        End If

        'Season # DVD
        If Not CInt(tEpisodeInfo.DVDSeason) = -1 Then
            nEpisode.SeasonDVD = tEpisodeInfo.DVDSeason
        End If

        'Season # Standard
        If Not CInt(tEpisodeInfo.SeasonNumber) = -1 Then
        ...
        With nAddonResult.ScraperResult_Data
            ...
            For Each aEpisode As TVDB.Model.Episode In Results.Series.Episodes
                ...
                If tScrapeModifiers.withEpisodes Then
                    Dim nEpisode As MediaContainers.MainDetails = GetInfo_TVEpisode(aEpisode, Results, tScrapeModifiers, tScrapeOptions)
                    .KnownEpisodes.Add(nEpisode)
                End If
            Next
        End With
        ...
        If nResult.Count > 0 Then
            For Each aShow In nResult
                nSearchResults.Add(New MediaContainers.MainDetails With {
                                   ...
                                   .Plot = aShow.Overview,
                                   .Premiered = CStr(aShow.FirstAired),
                                   .IMDB = aShow.IMDBId,
                                   .Title = aShow.Name,
                                   .TVDB = aShow.SeriesId})
