...
#Region "Methods"
    ...
    ''' </remarks>
    Public Function Scrape_Movie(ByVal strOriginalTitle As String, ByVal strTitle As String, ByVal strIMDB As String, ByVal tScrapeModifiers As Structures.ScrapeModifiers, ByVal tScrapeOptions As Structures.ScrapeOptions) As Interfaces.AddonResult
        ...
        Try
            ...
            If Not String.IsNullOrEmpty(strOriginalTitle) Then
                ...
                searchtitle = StringUtils.RemovePunctuation(strOriginalTitle).Replace("+", "-").Replace(" ", "-")
                ...
                If Not String.IsNullOrEmpty(sHtml) Then
                    TrailerWEBPageURL = "http://www.hd-trailers.net/movie/" & StringUtils.RemovePunctuation(strTitle).Replace("+", "-").Replace(" ", "-") & "/"
                    logger.Debug("[" & strOriginalTitle & "] HD-Trailer.net - Movie found on http://www.hd-trailers.net - URL: " & TrailerWEBPageURL)
                Else
                    'try with title instead of originaltitle!
                    sHTTP = New HTTP
                    sHtml = sHTTP.DownloadData("http://www.hd-trailers.net/movie/" & StringUtils.RemovePunctuation(strTitle).Replace("+", "-").Replace(" ", "-") & "/")
                    sHTTP = Nothing
                    If Not String.IsNullOrEmpty(sHtml) Then
                        TrailerWEBPageURL = "http://www.hd-trailers.net/movie/" & StringUtils.RemovePunctuation(strTitle).Replace("+", "-").Replace(" ", "-") & "/"
                        logger.Debug("[" & strOriginalTitle & "] HD-Trailer.net - Movie found on http://www.hd-trailers.net - URL: " & TrailerWEBPageURL)
                    Else
                        logger.Debug("[" & strOriginalTitle & "] HD-Trailer.net - Movie NOT found on http://www.hd-trailers.net - URL: " & "http://www.hd-trailers.net/movie/" & searchtitle & "/")
                        'Google search
                        'Warning: will be blocked by Google when spamming webrequests!
                        Dim BaseURL As String = "http://www.google.ch/search?q="
                        Dim searchTerm As String = "site:http://www.hd-trailers.net/ " + """" + strOriginalTitle + """"
                        ...
                        If Not String.IsNullOrEmpty(sHtml) Then
                            ...
                            If Not trailerWEBURLResults Is Nothing AndAlso trailerWEBURLResults.Count > 0 Then
                                ...
                                For Each searchresult As Match In trailerWEBURLResults
                                    ...
                                    If Not String.IsNullOrEmpty(TrailerWEBPageURL) Then
                                        If TrailerWEBPageURL.Contains("/") Then
                                            ...
                                            compareresult = 100
                                            searchtitle = (StringUtils.RemovePunctuation(strOriginalTitle.ToLower)).Replace(" ", "")
                                            ...
                                            If (strOriginalTitle.Length <= 5 AndAlso compareresult = 0) OrElse (strOriginalTitle.Length > 5 AndAlso compareresult <= 2) Then
                                                logger.Debug("[" & strOriginalTitle & "] HD-Trailer.net - Google: Movie found! URL: " & TrailerWEBPageURL)
                                                Exit For
                                            Else
                                                logger.Debug("[" & strOriginalTitle & "] HD-Trailer.net - Google: wrong result! URL: " & TrailerWEBPageURL)
                                                TrailerWEBPageURL = String.Empty
                                            End If
                                            compareresult = StringUtils.ComputeLevenshtein(comparestring, (StringUtils.RemovePunctuation(strTitle.ToLower)).Replace(" ", ""))
                                            If (strOriginalTitle.Length <= 5 AndAlso compareresult = 0) OrElse (strOriginalTitle.Length > 5 AndAlso compareresult <= 2) Then
                                                logger.Debug("[" & strOriginalTitle & "] HD-Trailer.net - Google: Movie found! URL: " & TrailerWEBPageURL)
                                                Exit For
                                            Else
                                ...
                                Next
                            Else
                                logger.Debug("[" & strOriginalTitle & "] HD-Trailer.net - Movie NOT found on http://www.hd-trailers.net using google! URL: " & SearchURL)
                            End If
                        Else
                            logger.Debug("[" & strOriginalTitle & "] HD-Trailer.net - Movie NOT found on http://www.hd-trailers.net using google! URL: " & SearchURL)
                        End If
                ...
                If Not String.IsNullOrEmpty(TrailerWEBPageURL) Then
                    logger.Info("[" & strOriginalTitle & "] HD-Trailer.net - Movie found! Download URL: " & TrailerWEBPageURL)
                    ...
                    If Not trailerURLResults Is Nothing AndAlso trailerURLResults.Count > 0 Then
                        ...
                        Next
                    Else
                        logger.Info("[" & strOriginalTitle & "] HD-Trailer.net - NO trailers found on specific movie page! URL: " & TrailerWEBPageURL)
                    End If
                Else
                    logger.Info("[" & strOriginalTitle & "] HD-Trailer.net - Movie NOT found")
                End If
            Else
                logger.Warn("[" & strOriginalTitle & "] HD-Trailer.net - Original title is empty, no scraping of trailers possible!")
            End If

            'log scraperesults for user
            If nAddonResult.ScraperResult_Trailers.Count < 1 Then
                logger.Info("[" & strOriginalTitle & "] HD-Trailer.net - NO trailer scraped!")
            Else
                logger.Info("[" & strOriginalTitle & "] HD-Trailer.net - Scraped " & nAddonResult.ScraperResult_Trailers.Count & " trailer!")
            End If
